...
def make_material(name, color, spec=0.5):
    ...
    return mat
skin_mat = make_material('Skin', (0.95, 0.8, 0.7))
...
bpy.ops.mesh.primitive_uv_sphere_add(radius=0.15, location=body_loc)
...
# Head
head_loc = (0, 0.19, body_loc[2] + 0.09)
...
# Snout
bpy.ops.mesh.primitive_cylinder_add(radius=0.04, depth=0.09, location=(0, head_loc[1] + 0.07, head_loc[2]))
snout = bpy.context.active_object
snout.data.materials.append(skin_mat)
bpy.ops.object.shade_smooth()

# Nostrils
for x in [-0.015, 0.015]:
    bpy.ops.mesh.primitive_uv_sphere_add(radius=0.008, location=(x, head_loc[1] + 0.12, head_loc[2] + 0.01))
...
# Ears
for x in [-0.05, 0.05]:
    bpy.ops.mesh.primitive_cone_add(radius1=0.025, depth=0.07, location=(x, head_loc[1] + 0.01, head_loc[2] + 0.07), rotation=(radians(-30), 0, 0))
...
for x in [-0.06, 0.06]:
    for y in [-0.08, 0.08]:
        bpy.ops.mesh.primitive_cylinder_add(radius=0.022, depth=0.15, location=(x, y, leg_z))
        leg = bpy.context.active_object
        leg.data.materials.append(skin_mat)
        bpy.ops.object.shade_smooth()

# Tail (curled)
bpy.ops.mesh.primitive_torus_add(location=(0, -0.13, body_loc[2] + 0.03), major_radius=0.025, minor_radius=0.008, rotation=(radians(90), 0, 0))
tail = bpy.context.active_object
tail.data.materials.append(skin_mat)
bpy.ops.object.shade_smooth()
